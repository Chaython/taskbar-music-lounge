...
// @compilerOptions -lole32 -ldwmapi -lgdi32 -luser32 -lwindowsapp -lshcore -lgdiplus -lshell32 -lshlwapi
// ==/WindhawkMod==
...
// --- Local GUID for IApplicationActivationManager ---
static const CLSID CLSID_AppActivationMgr = 
...
void UpdateMediaInfo() {
    try {
        ...
        if (session) {
            ...
            if (pos < 0) pos = 0;
            g_MediaState.positionTicks = pos;

            INT64 end = timeline.EndTime().count();
            INT64 start = timeline.StartTime().count();
            INT64 maxSeek = timeline.MaxSeekTime().count();
            INT64 dur = end - start;
            if (dur <= 0) dur = maxSeek;
            if (dur < 0) dur = 0;
            g_MediaState.durationTicks = dur;
        } else {
            lock_guard<mutex> guard(g_MediaState.lock);
...
// --- Fallback window search helpers ---
wstring ExtractExeFromAumid(const wstring& aumid) {
    size_t lastSlash = aumid.find_last_of(L"\\/");
    wstring basename = (lastSlash != wstring::npos) ? aumid.substr(lastSlash + 1) : aumid;
    size_t qm = basename.find(L'?');
    if (qm != wstring::npos) basename = basename.substr(0, qm);
    return basename;
}

struct EnumData {
    const wstring* exeName;
    HWND foundHwnd;
};
...
BOOL CALLBACK EnumWindowsForExe(HWND hwnd, LPARAM lParam) {
    ...
    if (hProcess) {
        ...
        if (QueryFullProcessImageNameW(hProcess, 0, path, &size)) {
            ...
            wstring exe = (lastSep != wstring::npos) ? fullPath.substr(lastSep + 1) : fullPath;
            if (StrStrIW(exe.c_str(), data->exeName->c_str()) != nullptr) {
                data->foundHwnd = hwnd;
...
void OpenSourceApp() {
    try {
        ...
        if (aumid.empty()) {
            Wh_Log(L"SourceAppUserModelId is empty, cannot open app");
            return;
        }
        Wh_Log(L"Activating app with AUMID: %ls", aumid.c_str());

        bool activated = false;

        IApplicationActivationManager* pActivateManager = nullptr;
        HRESULT hr = CoCreateInstance(CLSID_AppActivationMgr, nullptr,
                                      CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pActivateManager));
        if (SUCCEEDED(hr)) {
            DWORD processId = 0;
            hr = pActivateManager->ActivateApplication(aumid.c_str(), nullptr, AO_NONE, &processId);
            if (SUCCEEDED(hr)) {
                activated = true;
                Wh_Log(L"ActivateApplication succeeded, processId=%lu", processId);
            } else {
                Wh_Log(L"ActivateApplication failed: 0x%08X", hr);
            }
            pActivateManager->Release();
        } else {
            Wh_Log(L"CoCreateInstance for ActivationManager failed: 0x%08X", hr);
        }

        if (!activated) {
            wstring exeName = ExtractExeFromAumid(aumid.c_str());
            Wh_Log(L"Fallback: searching for window with executable containing '%s'", exeName.c_str());

            EnumData data;
            data.exeName = &exeName;
            data.foundHwnd = nullptr;
            EnumWindows(EnumWindowsForExe, (LPARAM)&data);

            if (data.foundHwnd) {
                SetForegroundWindow(data.foundHwnd);
                Wh_Log(L"Brought window to foreground (fallback)");
            } else {
                Wh_Log(L"Fallback window search failed");
            }
        }
    } catch (...) {
        Wh_Log(L"OpenSourceApp exception");
    }
}
    ...
    switch (msg) {
        ...
        case WM_APP + 10: {
            ...
            if (taskbarVisible && !IsWindowVisible(hwnd) && !g_IsHiddenByIdle) {
                ...
                int y = rcTaskbar.top + (taskbarHeight / 2) - (g_Settings.height / 2) + g_Settings.offsetY;
